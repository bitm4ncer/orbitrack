...
export function PresetSaveDialog({ params, color, onClose, onSaved }: PresetSaveDialogProps) {
  const presets = usePresetStore((s) => s.presets);
  const saveUserPreset = usePresetStore((s) => s.saveUserPreset);

  // Collect unique user folders
  ...
  const handleSave = async () => {
    if (!name.trim()) return;
    setSaving(true);
    const preset = await saveUserPreset(name.trim(), finalFolder, params);
    setSaving(false);
    onSaved(preset.id);
    onClose();
  };
  ...
  return createPortal(
    ...
    >
      <div
        ...
        style={{ background: '#14141e', width: 280 }}
      >
